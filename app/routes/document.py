...
def index() -> str:
    ...
    try:
        ...
        form.student_id.choices = [("", "-- Alle Studierende --")] + [
            (str(s.id), f"{s.last_name}, {s.first_name} ({s.student_id})")
            for s in students
        ]  # type: ignore
    ...
    try:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                flash(
                    f"Dokument '{document.original_filename}' erfolgreich hochgeladen.",
                    "success",
                )
        ...
        if form.validate_on_submit():
            ...
            for file in files:
                ...
                with tempfile.NamedTemporaryFile(
                    delete=False, suffix=Path(original_filename).suffix
                ) as tmp:
                    file.save(tmp.name)
                    tmp_path = tmp.name
                ...
                try:
                    service.upload_document(
                        file_path=tmp_path,
                        enrollment_id=int(matched_enrollment.id),
