...
class TestCourseIndexRoute:
    ...
    def test_index_empty(self, app, auth_client, course_service):
    ...
    def test_index_with_search(self, app, auth_client, course_service, sample_university):
        ...
        # Search by name
        response = auth_client.get("/courses/?search=Informatik")
        assert response.status_code == 200
        assert b"Informatik" in response.data
    ...
    def test_index_filter_by_semester(
        ...
        response = auth_client.get("/courses/?semester=2024_WiSe")
        assert response.status_code == 200
...
class TestCourseShowRoute:
    ...
    def test_show_existing_course(self, app, auth_client, course_service, sample_university):
        ...
        response = auth_client.get(f"/courses/{course.id}")
        assert response.status_code == 200
        assert b"Informatik" in response.data
        assert b"2024_WiSe" in response.data

    def test_show_nonexistent_course(self, app, auth_client, course_service):
...
class TestCourseNewRoute:
    ...
    def test_new_get(self, app, auth_client, course_service, sample_university):
        """Test GET request to new course form."""
        response = auth_client.get("/courses/new")
        assert response.status_code == 200
        assert b"Lehrveranstaltung" in response.data
...
class TestCourseEditRoute:
    ...
    def test_edit_get(self, app, auth_client, course_service, sample_university):
        ...
        response = auth_client.get(f"/courses/{course.id}/edit")
        assert response.status_code == 200
        assert b"bearbeiten" in response.data
        assert b"Informatik" in response.data

    def test_edit_get_nonexistent(self, app, auth_client, course_service):
...
class TestCourseDeleteRoute:
    ...
    def test_delete_get(self, app, auth_client, course_service, sample_university):
        ...
        response = auth_client.get(f"/courses/{course.id}/delete")
        assert response.status_code == 200
        assert b"schen" in response.data  # "löschen" with encoding
        assert b"Informatik" in response.data

    def test_delete_get_nonexistent(self, app, auth_client, course_service):
    ...
    def test_delete_post_nonexistent(self, app, auth_client, course_service):
        """Test POST request to delete non-existent course."""
        response = auth_client.post("/courses/999/delete", follow_redirects=False)
        assert response.status_code == 302
