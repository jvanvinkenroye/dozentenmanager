...
def enrollment_test_data(db):
    ...
    student = Student(
        ...
        last_name="Enroll",
        student_id="11223344",
        email="alice@enroll.com",
        program="CS",
    )
    db.session.add(student)
    db.session.commit()

    return {"university": uni, "course": course, "student": student}
...
def test_add_enrollment_student_not_found(enrollment_service, enrollment_test_data, db):
    with pytest.raises(
        ValueError, match="Student mit Matrikelnummer 99999999 nicht gefunden"
    ):
...
def test_update_enrollment_status(enrollment_service, enrollment_test_data, db):
    ...
    updated = enrollment_service.update_enrollment_status(
