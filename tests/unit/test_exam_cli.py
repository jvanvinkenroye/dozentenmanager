...
class TestAddExam:
    ...
    def test_add_exam_success(self, app, db, sample_course, exam_service):
    ...
    def test_add_exam_with_description(self, app, db, sample_course, exam_service):
    ...
    def test_add_exam_with_partial_weight(self, app, db, sample_course, exam_service):
    ...
    def test_add_exam_strips_whitespace(self, app, db, sample_course, exam_service):
    ...
    def test_add_exam_empty_name(self, app, db, sample_course, exam_service):
    ...
    def test_add_exam_name_too_long(self, app, db, sample_course, exam_service):
    ...
    def test_add_exam_invalid_course(self, app, db, exam_service):
    ...
    def test_add_exam_invalid_weight(self, app, db, sample_course, exam_service):
    ...
    def test_add_exam_description_too_long(self, app, db, sample_course, exam_service):
...
class TestListExams:
    ...
    @pytest.fixture
    def sample_exams(self, app, db, exam_service):
    ...
    def test_list_all_exams(self, app, db, sample_exams, exam_service):
        """Test listing all exams."""
        with app.app_context():
            exams = exam_service.list_exams()
            assert len(exams) == 3

    def test_list_exams_filter_by_course(self, app, db, sample_exams, exam_service):
        """Test filtering exams by course."""
        with app.app_context():
            course1_exams = exam_service.list_exams(course_id=sample_exams["course1"])
            assert len(course1_exams) == 2

            course2_exams = exam_service.list_exams(course_id=sample_exams["course2"])
            assert len(course2_exams) == 1

    def test_list_exams_empty(self, app, db, exam_service):
...
class TestGetExam:
    ...
    @pytest.fixture
    def sample_exam(self, app, db, exam_service):
        ...
        with app.app_context():
            ...
            exam = exam_service.add_exam("Klausur", course.id, date(2024, 6, 15), 100.0)
            return exam.id

    def test_get_exam_success(self, app, db, sample_exam, exam_service):
    ...
    def test_get_exam_not_found(self, app, db, exam_service):
        """Test getting non-existent exam returns None."""
        with app.app_context():
            exam = exam_service.get_exam(999)
            assert exam is None
...
class TestUpdateExam:
    ...
    def test_update_exam_name(self, app, db, sample_exam, exam_service):
        """Test updating exam name."""
        with app.app_context():
            exam = exam_service.update_exam(sample_exam, name="Final Exam")
            assert exam is not None
            assert exam.name == "Final Exam"

    def test_update_exam_date(self, app, db, sample_exam, exam_service):
        """Test updating exam date."""
        with app.app_context():
            new_date = date(2024, 7, 1)
            exam = exam_service.update_exam(sample_exam, exam_date=new_date)
            assert exam.exam_date == new_date

    def test_update_exam_max_points(self, app, db, sample_exam, exam_service):
        """Test updating max points."""
        with app.app_context():
            exam = exam_service.update_exam(sample_exam, max_points=120.0)
            assert exam.max_points == 120.0

    def test_update_exam_weight(self, app, db, sample_exam, exam_service):
        """Test updating weight."""
        with app.app_context():
            exam = exam_service.update_exam(sample_exam, weight=50.0)
    ...
    def test_update_exam_not_found(self, app, db, exam_service):
        """Test updating non-existent exam raises ValueError."""
        with app.app_context():
            with pytest.raises(ValueError, match="Exam with ID 999 not found"):
                exam_service.update_exam(999, name="New Name")

    def test_update_exam_empty_name(self, app, db, sample_exam, exam_service):
        """Test that empty name raises ValueError."""
        with app.app_context():
            with pytest.raises(ValueError, match="Exam name cannot be empty"):
                exam_service.update_exam(sample_exam, name="")
    ...
    def test_update_exam_invalid_weight(self, app, db, sample_exam, exam_service):
        """Test that invalid weight raises ValueError."""
        with app.app_context():
            with pytest.raises(ValueError, match="Weight must be between 0 and 100"):
                exam_service.update_exam(sample_exam, weight=150.0)
...
class TestDeleteExam:
    ...
    def test_delete_exam_success(self, app, db, sample_exam, exam_service):
        """Test deleting an exam successfully."""
        with app.app_context():
            result = exam_service.delete_exam(sample_exam)
    ...
    def test_delete_exam_not_found(self, app, db, exam_service):
        """Test deleting non-existent exam raises ValueError."""
        with app.app_context():
            with pytest.raises(ValueError, match="Exam with ID 999 not found"):
                exam_service.delete_exam(999)
