...
def exam_test_data(db):
    ...
    return {"university": uni, "course": course}
...
def test_add_exam_validation_error(exam_service, exam_test_data, db):
    ...
    with pytest.raises(ValueError, match="Weight must be between 0 and 100"):
        exam_service.add_exam(
            "Test", exam_test_data["course"].id, date.today(), 100.0, weight=101.0
        )


def test_list_exams(exam_service, exam_test_data, db):
    exam_service.add_exam(
        "Exam 1", exam_test_data["course"].id, date(2023, 1, 1), 100.0
    )
    exam_service.add_exam(
        "Exam 2", exam_test_data["course"].id, date(2023, 2, 1), 100.0
    )
...
def test_update_exam(exam_service, exam_test_data, db):
    exam = exam_service.add_exam(
        "Initial Name", exam_test_data["course"].id, date.today(), 100.0
    )
...
def test_delete_exam(exam_service, exam_test_data, db):
    exam = exam_service.add_exam(
        "Delete Me", exam_test_data["course"].id, date.today(), 100.0
    )
    exam_id = exam.id
