...
def test_add_student_duplicate_error(student_service, sample_student):
    # Duplicate ID
    with pytest.raises(
        ValueError, match="Student mit Matrikelnummer '12345678' existiert bereits"
    ):
        student_service.add_student(
            first_name="John",
            last_name="Doe",
    ...
    with pytest.raises(
        ValueError,
        match="Student mit E-Mail-Adresse 'max.mustermann@example.com' existiert bereits",
    ):
        student_service.add_student(
            first_name="John",
            last_name="Doe",
...
def test_update_student(student_service, sample_student):
    updated = student_service.update_student(
        sample_student.id, first_name="Maximilian", program="Data Science"
    )
